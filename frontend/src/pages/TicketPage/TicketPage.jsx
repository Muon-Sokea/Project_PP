...
import { apiGetTicket } from '../../services/registration.service.js';
...
function getStoredTicket() {
  try {
    ...
    const ticketType = localStorage.getItem('erms_ticket_type') || 'General Admission';
    ...
    return { code, ticketType, event, user };
  } catch {
    return { code: null, ticketType: 'General Admission', event: null, user: null };
  }
}
...
export default function TicketPage() {
  ...
  const [ticket,      setTicket]      = useState({ code: null, ticketType: 'General Admission', event: null, user: null });
    ...
    if (stored.code) {
      apiGetTicket(stored.code)
        .then(apiTicket => {
          ...
          setTicket(prev => ({
            code: prev.code,
            ticketType: apiTicket.ticketType || prev.ticketType,
  ...
  async function handleDownload() {
    ...
    try {
      ...
      // Scale down if taller than one page
      if (drawH > pageH - 20) {
        drawH = pageH - 20;
      }
    ...
    <div className="ticket-page">
      <div className="ticket-card" ref={ticketCardRef}>
        ...
        <div className="ticket-details">
          ...
          <div className="ticket-detail-item">
            <div className="ticket-detail-icon"><i className={ticket.ticketType?.toLowerCase().includes('vip') ? 'ri-vip-diamond-line' : 'ri-tag-line'} /></div>
            <div className="ticket-detail-content">
              <span className="ticket-detail-label">Ticket Type</span>
              <span className="ticket-detail-value">{ticket.ticketType || 'General Admission'}</span>
            </div>
          </div>
