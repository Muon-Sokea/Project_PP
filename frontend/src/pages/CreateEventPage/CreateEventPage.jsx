...
export default function CreateEventPage() {
  ...
  async function publish() {
    ...
    try {
      ...
      alert('Failed to save event. Check your connection and try again.');
      setSubmitting(false);
      return;
    }
    setSubmitting(false);
  ...
  async function handleImageFile(file) {
    ...
    // Upload to backend with YYYYMMDDHHMMSS naming
    try {
      const result = await apiUploadImage(file);
      // Replace preview with the server URL once uploaded
      if (result?.url) {
